...
resource "azurerm_availability_set" "director_aset" {
  count = 1
  name = "${var.organization}-${var.project}-${var.environment}-director-aset"
  location = "${var.region}"
  resource_group_name = "${var.resource_group}"
}

# Subnets as per defined; Directors start at xx.xx.50.yy
resource "azurerm_subnet" "director_subnet" {
  ...
  address_prefix = "${cidrsubnet(var.cidr_block, 8, 50 + count.index + 1)}"
...
resource "azurerm_network_interface" "director_netif" {
  count = "${var.num_servers}"

  name = "${var.organization}-${var.project}-${var.environment}-director-netif-${count.index + 1}"
  resource_group_name = "${var.resource_group}"
  location = "${var.region}"
...
resource "azurerm_virtual_machine" "director_node" {
  count = "${var.num_servers}"
  name = "${var.organization}-${var.project}-${var.environment}-director-node-${count.index + 1}"
  location = "${var.region}"
  resource_group_name = "${var.resource_group}"
  network_interface_ids = [
    "${element(azurerm_network_interface.director_netif.*.id, count.index)}"]
  # vm_size = "Standard_A0"
  ...
  storage_os_disk {
    name = "${var.organization}-${var.project}-${var.environment}-director-osdisk-${count.index + 1}"
    vhd_uri = "${var.storage_uri}/${var.organization}-${var.project}-${var.environment}-director-osdisk-${count.index + 1}.vhd"
    caching = "ReadWrite"
  ...
  os_profile {
    computer_name = "${var.organization}-${var.project}-${var.environment}-director-node-${count.index + 1}"
    admin_username = "testadmin"
    admin_password = "Password1234!"
    custom_data = "${base64encode(file("director-cloud-init.txt"))}"
  }
  ...
  availability_set_id = "${(var.num_servers * 1 > 1) ? azurerm_availability_set.director_aset.id : ""}"
